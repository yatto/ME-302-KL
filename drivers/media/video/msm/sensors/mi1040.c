...
int32_t mi1040_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(machine_is_apq8064_duma()){
		...
		if (rc != 0)
			pr_err("gpio 12 CAM_RST config low fail\n");
		else
			pr_info("gpio 12 CAM_RST(%d)\n", gpio_get_value(CAM_RST_DUMA));
		rc = pm8xxx_gpio_config(CAM_PWDN, &pm_isp_gpio_low);
		if (rc != 0)
			pr_err("gpio 10 CAM_PWDN config low fail\n");
		else
			pr_info("gpio 10 CAM_PWDN(%d)\n", gpio_get_value(CAM_PWDN));
	}
	else{
	...
	if(machine_is_apq8064_duma()){
		...
		if (rc != 0)
			pr_err("gpio 12 CAM_RST config high fail\n");
		else
			pr_info("gpio 12 CAM_RST(%d)\n", gpio_get_value(CAM_RST_DUMA));
		msleep(5);
		rc = pm8xxx_gpio_config(CAM_PWDN, &pm_isp_gpio_high);
		if (rc != 0)
			pr_err("gpio 10 CAM_PWDN config high fail\n");
		else
			pr_info("gpio 10 CAM_PWDN(%d)\n", gpio_get_value(CAM_PWDN));
	}
	else{
