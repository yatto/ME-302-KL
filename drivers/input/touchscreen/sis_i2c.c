...
#define GPIO_IRQ 6 // GPIO_6
#define GPIO_TP_PW 7
#define GPIO_TP_RESET 31
...
static long sis_stress_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int err = 1;
...
static int sis_read_firmware_info(struct sis_ts_data *ts){  
    ...
    unsigned char fw_info_cmd[14] = {0x04, 0x00, 0x0C, 0x00, 0x09, 0x00, 0x86,   
...
static ssize_t sis_show_tp_vendor(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	*/
	int vendor_id = asustek_get_tp_type();
	return sprintf(buf, "%02d", vendor_id);
}
...
/*	if(machine_is_apq8064_seamullet()||machine_is_apq8064_greymullet()){
       	//get ts power
...
*/	//hw reset
/*	if(machine_is_apq8064_duma()||machine_is_apq8064_seamullet()||machine_is_apq8064_greymullet())
	{
	    gpio_direction_output(GPIO_TP_RESET, 0); 
     	    msleep(5);
...
#endif
if(machine_is_apq8064_duma()){
	gpio_direction_output(GPIO_TP_PW, 0);//close the tp power
}
else/* if(machine_is_apq8064_seamullet()||machine_is_apq8064_greymullet())*/{
	
	rc = regulator_disable(pm8921_l17_ts);
	if (rc) {
...
if(machine_is_apq8064_duma()){
    gpio_direction_output(GPIO_TP_PW, 1);//turn on the tp power
    msleep(20);      	
    gpio_direction_output(GPIO_TP_RESET, 0); // touch reset 
    msleep(5);
    gpio_direction_output(GPIO_TP_RESET, 1);
}
else /*if(machine_is_apq8064_seamullet()||machine_is_apq8064_greymullet())*/{

    //enable LDO17 for ts power
